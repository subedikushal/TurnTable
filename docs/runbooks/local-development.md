...
1. Copy `.env.example` to `.env` and choose OIDC or strictly local development authentication.
...
5. Start the API and worker with `pnpm dev`, or run them separately with filtered commands from the root README.
